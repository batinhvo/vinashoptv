<template>
    <div class="flex flex-wrap text-center py-4 mt-8 border-t border-gray-100">
        <div class="w-full lg:w-1/2 lg:text-left">
            <p>Showing {{ from }}-{{ to }} of {{ data.dataTotal }} results</p>
        </div>
...
<script setup lang="ts">
    ...
    const to = computed(() => Math.min(page.value * itemsPerPage, data.dataTotal));

    const pages = computed(() => Math.ceil((data.dataTotal ?? 1) / itemsPerPage));
