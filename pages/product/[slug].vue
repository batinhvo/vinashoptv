        ...
        <div class="container mx-auto" v-if="dataDetails">
            ...
            <div class="my-0 md:my-10 overflow-x-auto whitespace-nowrap max-w-[450px] md:max-w-full">
                <ul class="flex items-center bg-zinc-100 md:bg-white py-2 xl:py-0 pl-2 xl:pl-0">
                    ...
                    <li class="px-3 py-2 md:bg-zinc-100 hover:bg-gray-200 rounded md:border md:border-gray-100"><NuxtLink :to="`/categories/${cateSlug}`">{{ cateTitle }}</NuxtLink></li>
                    <li class="px-2"><i class="ec ec-arrow-right-categproes"></i></li>
...
<script setup lang="ts">
    ...
    const fetchDataProduct = async () => {
        ...
        cateTitle.value = cateStore.categories.find((cate) => cate.id === dataDetails.value?.categoryId)?.name || '';
        cateSlug.value = cateStore.categories.find((cate) => cate.id === dataDetails.value?.categoryId)?.slug || '';
    }
